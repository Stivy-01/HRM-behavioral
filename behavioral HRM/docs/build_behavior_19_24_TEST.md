## Build a tiny Behavior dataset limited to 19:00–24:00

This guide shows how to:
- create a small builder script that filters events by `event_start_datetime` hour ∈ [19, 24)
- run it on one or more baseline SQLite files
- validate the output using the existing validator

Requirements: Windows PowerShell, Python 3.9+.

### 1) Open PowerShell in the repo root

```powershell
cd "C:\Users\andre\Desktop\2025 tesi\HRM-behavioral"
```
...
This writes a new script to `behavioral HRM\scripts\build_windowed_from_sqlite.py`.
...
if __name__ == "__main__":
    main()
"@ | Set-Content -Encoding UTF8 "behavioral HRM\scripts\build_windowed_from_sqlite.py"
```

### 3) Run it on your SQLite baseline DB

Set your inputs and output folder. For multiple DBs, pass them all after `--sqlite`.

```powershell
$DB = "C:\path\to\your\baseline.db"   # change this
$OUT = "behavioral HRM\data\behavior-test-19-24"

python "behavioral HRM\scripts\build_windowed_from_sqlite.py" `
  --sqlite $DB `
  --output_root $OUT `
  --hour_start 19 `
  --hour_end 24 `
  --seq_len 256 `
  --stride 128 `
  --include_time_tokens 1
```

Notes:
- `--hour_start 19 --hour_end 24` keeps events whose `event_start_datetime` hour is 19, 20, 21, 22, or 23.
- Everything is written to `$OUT\train\...` (val/test left empty for this smoke test).

### 4) Validate the dataset layout

```powershell
$env:HRM_BEHAVIOR_DATA = (Resolve-Path $OUT).Path
python "behavioral HRM\scripts\validate_behavior_dataset.py"
```

You should see shapes for the `train` split and a token preview. If you see zero examples, double-check your DB path and that the window 19–24 actually has events.